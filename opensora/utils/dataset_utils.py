...
class Collate:
    ...
    def __call__(self, batch):
        ...
        if self.num_frames > 1 and self.use_image_num == 0:
            pad_batch_tubes, attention_mask = self.process(batch_tubes_vid,
                                                           t_ds_stride,
                                                           ds_stride,
                                                           self.max_thw,
                                                           self.ae_stride_thw,
            ...
            pad_batch_tubes_vid, attention_mask_vid = self.process(batch_tubes_vid,
                                                                   t_ds_stride,
                                                                   ds_stride,
                                                                   self.max_thw,
                                                                   self.ae_stride_thw,
            ...
            pad_batch_tubes_img, attention_mask_img = self.process(batch_tubes_img,
                                                                   1,
                                                                   ds_stride,
                                                                   self.max_1hw,
                                                                   self.ae_stride_1hw,
            ...
            pad_batch_tubes = torch.cat([pad_batch_tubes_vid, pad_batch_tubes_img],
                                        dim=2)  # concat at temporal, video first
    ...
    def process(self, batch_tubes, t_ds_stride, ds_stride, max_thw, ae_stride_thw, patch_size_thw, extra_1):
        ...
        max_latent_size = [((max_tube_size[0] - 1) // ae_stride_thw[0] + 1) if extra_1 else
                           (max_tube_size[0] // ae_stride_thw[0]), max_tube_size[1] // ae_stride_thw[1],
        ...
        attention_mask = [
            F.pad(torch.ones(i),
                  ...
                  value=0) for i in valid_latent_size
        ]
