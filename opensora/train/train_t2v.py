...
from opensora.models.diffusion.diffusion import \
    create_diffusion_T as create_diffusion
...
def save_state(model, optimizer, epoch, filepath):
...
def main(args):
    ...
    # (Model Init 1) Create diffusion (scheduler):
    diffusion = create_diffusion(timestep_respacing="")  # default: 1000 steps, linear noise schedule
    kwargs = {}

    # (Model Init 2) Get Causal Autoencoder
    ...
    # (Model Init 3) Get Text Autoencoder
    ...
    # Movevae and text_encoder to device and cast to weight_dtype
    # The VAE is in float32 to avoid NaN losses.
    ae.to('cuda', dtype=weight_dtype)
    text_enc.to('cuda', dtype=weight_dtype)
    model.cuda()
    ...
    for epoch in range(first_epoch, args.num_train_epochs):
        ...
        for step, (x, attn_mask, input_ids, cond_mask) in enumerate(train_dataloader):
            ...
            x = x.to(device, dtype=weight_dtype)  # B C T+num_images H W, 16 + 4
            attn_mask = attn_mask.to(device)  # B 1+num_images L
            # assert torch.all(attn_mask != 0), 'attn_mask must all 1'
            input_ids = input_ids.to(device)  # B 1+num_images L
            cond_mask = cond_mask.to(device)  # B 1+num_images L
            ...
            if global_step % LOG_INTERVAL == 1:
                logger.info(f"[{global_step}] train_loss: [{loss.item()}]")
